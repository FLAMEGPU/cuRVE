#ifndef INCLUDE_UTIL_CUDAEVENTTIMER_CUH_
#define INCLUDE_UTIL_CUDAEVENTTIMER_CUH_
 ...
 private:
    ...
    void sync() {
        ...
        if (!startEventRecorded) {
            throw std::runtime_error("CUDAEventTimer::start() must be called prior to getElapsed*");
        }
        if (!stopEventRecorded) {
            throw std::runtime_error("CUDAEventTimer::stop() must be called prior to getElapsed*");
        }
...
#endif  // INCLUDE_UTIL_CUDAEVENTTIMER_CUH_
